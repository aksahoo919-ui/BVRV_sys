...
export default function MentorDefaulters() {
  ...
  async function load() {
    ...
    try {
      const r = await api.get('/mentor/defaulters');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Students below {threshold}% attendance in your weekly class.</p>
      </div>
        ...
        <div className="card overflow-x-auto p-0">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {defaulters.length === 0 ? (
                <tr><td colSpan={3} className="text-center py-10 text-emerald-600">🎉 No defaulters.</td></tr>
              ) : defaulters.map(s => (
