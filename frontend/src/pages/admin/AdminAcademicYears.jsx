...
export default function AdminAcademicYears() {
  ...
  async function handleSave(e) {
    ...
    try {
      if (modal.mode === 'add') {
        ...
        await api.patch(`/admin/academic-years/${modal.data.id}`, form);
      }
      setModal(null);
      load();
    } catch (e) {
  ...
  async function handleDelete(row) {
    if (!window.confirm(`Delete academic year "${row.label}"?`)) return;
    try {
      await api.delete(`/admin/academic-years/${row.id}`);
      load();
    } catch (e) {
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="btn-primary" onClick={openAdd}>+ Add Academic Year</button>
      </div>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={5} className="py-6 text-center text-gray-400">No academic years yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="card w-full max-w-md shadow-xl">
            <h2 className="text-lg font-semibold mb-4">{modal.mode === 'add' ? 'Add Academic Year' : 'Edit Academic Year'}</h2>
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-sm font-medium mb-1">Label (e.g. 2024–25)</label>
                <input className="input w-full" required value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} />
              </div>
              <div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm">Mark as current year</span>
              </label>
